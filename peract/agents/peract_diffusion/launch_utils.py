...
from peract.agents.peract_diffusion.perceiver_lang_io import PerceiverVoxelLangEncoder
from peract.agents.peract_diffusion.qattention_peract_bc_agent import (
    QAttentionPerActBCAgent,
)
from peract.agents.peract_diffusion.qattention_stack_agent import QAttentionStackAgent
from helpers import demo_loading_utils, utils
from helpers.clip.core.clip import build_model, load_clip, tokenize
from helpers.preprocess_agent import PreprocessAgent
from train_low_level import _create_agent_fn as _create_diffusion
...
def create_diffusion(cfgs: DictConfig):
    device = "cuda"
    robot, diffusion = _create_diffusion(cfgs, device=device)
...
def create_agent(cfg: DictConfig):
    ...
    diffusion_model, robot = create_diffusion(cfg.method)
    rotation_agent = QAttentionStackAgent(
        qattention_agents=qattention_agents,
        diffusion_agent=diffusion_model,
        rotation_resolution=cfg.method.rotation_resolution,
        camera_names=cfg.rlbench.cameras,
        robot=robot,
        scene_bounds=cfg.method.scene_bounds,
        diff_model_path=cfg.method.model_path,
        diff_gripper="open" in cfg.rlbench.tasks[0] or "toilet" in cfg.rlbench.tasks[0],
    )
